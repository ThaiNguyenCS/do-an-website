...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`https://domstore.azurewebsites.net/api/v1/products/${productId}`);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await axios.post("https://domstore.azurewebsites.net/api/v1/carts", cartData);
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <div className="md:w-1/2">
          ...
          <div className="flex justify-center space-x-2">
            {product.imageUrl.map((img, index) => (
              ...
              >
                <img src={img} alt={`Hình thu nhỏ ${index + 1}`} className="w-full h-full object-cover" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="md:w-1/2">
          ...
          <div className="mb-4">
            ...
            {product.promotionalPrice && (
              <span className="ml-2 text-lg text-gray-500 line-through">
                {product.promotionalPrice.toLocaleString("vi-VN", { style: "currency", currency: "VND" })}
              </span>
            )}
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {product.colorSummary.map((colorObj) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full border-2 ${selectedColor === colorObj.color ? "border-blue-500" : "border-gray-300"}`}
          ...
          <button
            ...
            className={`w-full ${addingToCart ? "bg-blue-400" : "bg-blue-600"} text-white py-3 rounded-lg font-semibold text-lg hover:bg-blue-700 transition duration-300 flex items-center justify-center`}
          >
            {addingToCart ? (
          ...
          <div className="mt-4">
            ...
            <p><strong>Tổng số lượng:</strong> {product.totalQuantity}</p>
            <p><strong>Tổng số màu:</strong> {product.totalColors}</p>
            <p><strong>Tổng số kích thước:</strong> {product.totalSizes}</p>
          </div>
        </div>
      </div>
