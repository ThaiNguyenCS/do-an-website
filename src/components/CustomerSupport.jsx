...
const apiURL =
  "https://domstore.azurewebsites.net/api/v1/customer/support-tickets";
...
const CustomerSupport = () => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        setTickets(
          tickets.map((ticket) =>
            ticket._id === editTicket._id ? editTicket : ticket,
          ),
        );
        setIsEditing(false);
        setEditTicket(null);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 110,
      render: (status) => (
        <Tag color={status === "finish" ? "green" : "yellow"}>{status}</Tag>
      ),
    },
    {
      title: "",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200 p-6 flex flex-col">
      ...
      <h2 className="font-bold text-4xl text-center mb-6 text-gray-800 tracking-tight">
        Hỗ trợ khách hàng
      </h2>
