...
const VoucherManagement = () => {
  ...
  const handleEditVoucher = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`${API_BASE_URL}/vouchers/${selectedVoucher._id}`, {
  ...
  const handleDeactivateExpired = async () => {
    setLoading(true);
    try {
      await axios.post(`${API_BASE_URL}/vouchers/deactivate-expired`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`
        }
      });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button onClick={() => setIsCreateModalOpen(true)} className="bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-600">
            <FiPlusCircle /> Tạo Voucher
          </button>
          <button onClick={handleDeactivateExpired} className="bg-red-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-red-600">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {vouchers.map((voucher) => (
              <tr key={voucher._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{new Date(voucher.expirationDate).toLocaleDateString()}</td>
                <td className="px-6 py-4 whitespace-nowrap">{voucher.quantity}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex gap-2">
                    <button onClick={() => { setSelectedVoucher(voucher); setIsViewModalOpen(true); }} className="text-blue-500 hover:text-blue-700">
                      <FiEye />
                    </button>
                    <button onClick={() => { setSelectedVoucher(voucher); setFormData({ ...voucher, expirationDate: new Date(voucher.expirationDate).toISOString().split("T")[0] }); setIsEditModalOpen(true); }} className="text-green-500 hover:text-green-700">
                      <FiEdit2 />
                    </button>
                    <button onClick={() => { setSelectedVoucher(voucher); setIsDeleteModalOpen(true); }} className="text-red-500 hover:text-red-700">
                      <FiTrash2 />
                    </button>
                    <button onClick={() => { setSelectedVoucher(voucher); fetchVoucherStats(voucher._id); setIsStatsModalOpen(true); }} className="text-purple-500 hover:text-purple-700">
                      <BsGraphUp />
                    </button>
                  </div>
                </td>
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Delete Voucher">
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <button onClick={handleDeleteVoucher} disabled={loading} className="flex-1 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 disabled:bg-red-300">
              {loading ? "Deleting..." : "Xóa"}
            </button>
            <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600"> Hủy </button>
          </div>
        </div>
      </Modal>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-orange-100 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-orange-700">{Math.round((stats.usedCount / stats.totalQuantity) * 100)}%</p>
              </div>
            </div>
          </div>
        )}
